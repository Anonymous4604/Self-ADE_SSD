...
class SSD(nn.Module):
    def __init__(self, cfg,
        ...
        if cfg.MODEL.SELF_SUPERVISED:
            ...
            if cfg.MODEL.SELF_SUPERVISOR.TYPE == "jigen":
                self.ss_classifier = nn.Linear(n_out * 3 * 3, cfg.MODEL.SELF_SUPERVISOR.CLASSES + 1)
            else:
    ...
    def load(self, model):
        ...
        if "model_state_dict" in checkpoint:
            # this is really a checkpoint also containing other info
...
class MatchPrior(object):
    ...
    def __call__(self, gt_boxes, gt_labels):
        ...
        if (gt_boxes.shape[0] == 0):
            raise ValueError("Error in ground truth box size: " + str(gt_boxes.shape))
