...
def _evaluation(cfg, dataset_name, test_dataset, predictor, distributed, output_dir):
    ...
    for i in progress_bar(indices):
        image = test_dataset.get_image(i, apply_transform=False)
...
def do_evaluation(cfg, model, output_dir, distributed, datasets_dict=None):
    ...
    model.eval()
    predictor = Predictor(cfg=cfg, iou_threshold=cfg.TEST.NMS_THRESHOLD, score_threshold=cfg.TEST.CONFIDENCE_THRESHOLD,
    ...
    if cfg.TEST.MODE == "split":
        ...
        metric = _evaluation(cfg, joint_dataset_name, test_datasets, predictor, distributed, output_dir)
